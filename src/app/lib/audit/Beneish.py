...
class BeneishMScoreCalculator:
    ...
    def _compute_previous_years(self):
        ...
        financial_fields = [
            ...
            'PPE', 'Net PPE', 'Securities', 'Total Assets',
    ...
    def _calculate_indices(self):
        ...
        df['DSRI'] = (df['Net Receivables'] / df['Sales']) / (df['Net Receivables_prev'] / df['Sales_prev'])

        # GMI: Gross Margin Index
        df['Gross_Margin'] = (df['Sales'] - df['Cost of Goods Sold']) / df['Sales']
        df['Gross_Margin_prev'] = (df['Sales_prev'] - df['Cost of Goods Sold_prev']) / df['Sales_prev']
        df['GMI'] = df['Gross_Margin_prev'] / df['Gross_Margin']

        # AQI: Asset Quality Index
        df['Asset_Quality'] = 1 - (df['Current Assets'] + df['Net PPE'] + df['Securities']) / df['Total Assets']
        df['Asset_Quality_prev'] = 1 - (df['Current Assets_prev'] + df['Net PPE_prev'] + df['Securities_prev']) / df['Total Assets_prev']
        ...
        df['Depreciation_Rate'] = df['Depreciation Expense'] / (df['Depreciation Expense'] + df['Net PPE'])
        df['Depreciation_Rate_prev'] = df['Depreciation Expense_prev'] / (df['Depreciation Expense_prev'] + df['Net PPE_prev'])
    ...
    def get_results(self) -> pd.DataFrame:
        ...
        columns_to_include = [
            'Company', 'Year', 'DSRI', 'GMI', 'AQI',
            'SGI', 'DEPI', 'SGAI', 'TATA', 'LVGI', 'M-Score'
